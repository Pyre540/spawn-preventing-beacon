package pyre.spawnpreventingbeacon.gui;
...
public class CustomBeaconGUI extends GuiContainer {
    private static final ResourceLocation BEACON_GUI_TEXTURES = new ResourceLocation("textures/gui/container/beacon.png");
    private final IInventory tileBeacon;
    private CustomBeaconGUI.ConfirmButton beaconConfirmButton;
    private boolean buttonsNotDrawn;

    public CustomBeaconGUI(Container container, IInventory tileBeaconIn) {
        super(container);
    ...
    public void initGui() {
        super.initGui();
        this.beaconConfirmButton = new CustomBeaconGUI.ConfirmButton(-1, this.guiLeft + 164, this.guiTop + 107);
        this.buttonList.add(this.beaconConfirmButton);
        this.buttonList.add(new CustomBeaconGUI.CancelButton(-2, this.guiLeft + 190, this.guiTop + 107));
        ...
        if (this.buttonsNotDrawn && i >= 0) {
            ...
            for (int k = 0; k <= 2; ++k) {
                ...
                for (int j1 = 0; j1 < l; ++j1) {
                    ...
                    CustomBeaconGUI.PowerButton guibeacon$powerbutton = new CustomBeaconGUI.PowerButton(j++, this.guiLeft + 76 + j1 * 24 - i1 / 2, this.guiTop + 22 + k * 25, potion2, k);
            ...
            for (int j2 = 0; j2 < l1 - 1; ++j2) {
                ...
                CustomBeaconGUI.PowerButton guibeacon$powerbutton2 = new CustomBeaconGUI.PowerButton(j++, this.guiLeft + 167 + j2 * 24 - i2 / 2, this.guiTop + 47, potion3, 3);
            ...
            if (potion != null && potion != ModPotions.SPAWN_PREVENTING) {
                CustomBeaconGUI.PowerButton guibeacon$powerbutton1 = new CustomBeaconGUI.PowerButton(j++, this.guiLeft + 167 + (l1 - 1) * 24 - i2 / 2, this.guiTop + 47, potion, 3);
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == -2) {
            ...
            this.mc.player.connection.sendPacket(new CPacketCloseWindow(this.mc.player.openContainer.windowId));
            this.mc.displayGuiScreen(null);
        } else if (button instanceof CustomBeaconGUI.PowerButton) {
            CustomBeaconGUI.PowerButton guibeacon$powerbutton = (CustomBeaconGUI.PowerButton) button;
    ...
    static class Button extends GuiButton {
        ...
        protected Button(int buttonId, int x, int y) {
            super(buttonId, x, y, 22, 22, "");
        }
        ...
        public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
            if (this.visible) {
                mc.getTextureManager().bindTexture(CustomBeaconGUI.BEACON_GUI_TEXTURES);
                GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                ...
                if (!CustomBeaconGUI.BEACON_GUI_TEXTURES.equals(this.iconTexture)) {
                    mc.getTextureManager().bindTexture(this.iconTexture);
                }

                if (this.iconTexture == ModPotions.SPAWN_PREVENTING.getIconTexture()) {
                    Gui.drawModalRectWithCustomSizedTexture(x + 2, y + 2, 0, 0, 18, 18, 18, 18);
                } else {
        ...
        public void setIconTexture(ResourceLocation iconTexture, int iconX, int iconY) {
    ...
    @SideOnly(Side.CLIENT)
    class CancelButton extends CustomBeaconGUI.Button {
        public CancelButton(int buttonId, int x, int y) {
            super(buttonId, x, y, CustomBeaconGUI.BEACON_GUI_TEXTURES, 112, 220);
        }

        @Override
        public void drawButtonForegroundLayer(int mouseX, int mouseY) {
            CustomBeaconGUI.this.drawHoveringText(I18n.format("gui.cancel"), mouseX, mouseY);
        }
    }

    @SideOnly(Side.CLIENT)
    class ConfirmButton extends CustomBeaconGUI.Button {
        public ConfirmButton(int buttonId, int x, int y) {
            super(buttonId, x, y, CustomBeaconGUI.BEACON_GUI_TEXTURES, 90, 220);
        }

        @Override
        public void drawButtonForegroundLayer(int mouseX, int mouseY) {
            CustomBeaconGUI.this.drawHoveringText(I18n.format("gui.done"), mouseX, mouseY);
        }
    }

    @SideOnly(Side.CLIENT)
    class PowerButton extends CustomBeaconGUI.Button {
        private final Potion effect;
        private final int tier;

        public PowerButton(int buttonId, int x, int y, Potion effectIn, int tierIn) {
            super(buttonId, x, y);
            if (effectIn == ModPotions.SPAWN_PREVENTING) {
                setIconTexture(ModPotions.SPAWN_PREVENTING.getIconTexture(), 0, 0);
            } else {
                setIconTexture(GuiContainer.INVENTORY_BACKGROUND, effectIn.getStatusIconIndex() % 8 * 18, 198 + effectIn.getStatusIconIndex() / 8 * 18);
            }
